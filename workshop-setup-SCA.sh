...
#       echo -e "\norgInd=$orgInd, APIKEY=${WS_APIKEYS[$orgInd]}, USERKEY=${WS_USERKEYS[$orgInd]}, emailsPerOrg=$emailsPerOrg, assignedUsers=$assignedUsers"

        echo -e "\nOrg #$orgInd: Invite Users to an Organization"
        curl --request POST -H "Content-Type:application/json" $WS_WSS_URL \
       -d "{'requestType':'inviteUsers', 'userKey':'${WS_USERKEYS[$orgInd]}', 'orgToken':'${WS_APIKEYS[$orgInd]}', 'inviter':{'email':'$WS_INVITER'}, 'emails':[$emailsPerOrg]}"

        echo -e "\nOrg #$orgInd: Add Users to ${WS_USER_GROUP} Group"
        curl --request POST -H "Content-Type:application/json" $WS_WSS_URL \
        -d "{'requestType':'addUsersToGroups', 'userKey':'${WS_USERKEYS[$orgInd]}', 'orgToken':'${WS_APIKEYS[$orgInd]}', 'assignedUsers':[[{'name':'$WS_USER_GROUP'}, [${assignedUsers}$
